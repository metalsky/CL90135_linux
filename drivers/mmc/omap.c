...
static int __init mmc_omap_probe(struct platform_device *pdev)
{
	...
	printk(KERN_WARNING "*** mmc omap probe\n");

	if (minfo == NULL) {
		dev_err(&pdev->dev, "platform data missing\n");
		return -ENXIO;
	}
	printk(KERN_WARNING "*** mmc omap probe pass 1\n");

	res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	irq = platform_get_irq(pdev, 0);
	if (res == NULL || irq < 0)
		return -ENXIO;
	printk(KERN_WARNING "*** mmc omap probe pass 2\n");

	res = request_mem_region(res->start, res->end - res->start + 1,
				 pdev->name);
	if (res == NULL)
		return -EBUSY;
	printk(KERN_WARNING "*** mmc omap probe pass 3\n");
	...
	printk(KERN_WARNING "*** mmc omap probe pass 4\n");
