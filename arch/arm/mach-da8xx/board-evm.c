...
//  
//  MICHELE: DEFINIZIONE GPIO 2_13 COME USCITA DI RESET FPGA
//
//      RESET=0--> FPGA SOTTO RESET
//      RESET=1--> FPGA NON SOTTO RESET
//
#define ON_BD_RESET_FPGA	GPIO(45)	// GPIO2[13]
...
// definizione dei vari led a disposizione
TipoStruct_Initialize_CL90135_leds init_CL90135_leds[enum_CL90135_led_numOf]={
    {GPIO(4*16+4),1, "ON_BD_CL90135_RED_LED"   }, // red     led
    {GPIO(4*16+5),0, "ON_BD_CL90135_GREEN1_LED"}, // green 1 led
    {GPIO(4*16+6),1, "ON_BD_CL90135_GREEN2_LED"}, // green 2 led
    {GPIO(4*16+7),1, "ON_BD_CL90135_GREEN3_LED"}  // green 3 led
};
...
#if 1
    // Michele: scheda CL90135 non ha per il momento gestione del read only
    static int CL90135_mmc_get_ro(int index)
    {
	    return 0;
    }
    static struct davinci_mmc_platform_data CL90135_mmc_pdata = {
	        .mmc_clk		    = "MMC_SD_CLK",
	        .rw_threshold		= 512*4,
	        .use_4bit_mode		= 1,
	        .use_8bit_mode		= 0,
...
static void *da8xx_evm_soc_pdata[DA8XX_PDEV_COUNT] __initdata = {
	...
	[DA8XX_PDEV_MMC]	= &CL90135_mmc_pdata,
...
static int CL90135_emifA_fpga_init(void)
{
#define	SZ_32K	(32 * 1024)
	/*
	 * Turn on AEMIF clocks
	 */
	CL90135_emifA_fpga.clk = clk_get(NULL, "AEMIFCLK");
	if (IS_ERR(CL90135_emifA_fpga.clk)) {
		printk(KERN_ERR "%s: could not get AEMIF clock\n", __func__);
		CL90135_emifA_fpga.clk = NULL;
		return -ENODEV;
	}
	clk_enable(CL90135_emifA_fpga.clk);

	CL90135_emifA_fpga.aemif.res = request_mem_region(DA8XX_EMIF25_CONTROL_BASE,SZ_32K, "AEMIF control");
	if (CL90135_emifA_fpga.aemif.res == NULL) {
		printk(KERN_ERR	"%s: could not request AEMIF control region\n", __func__);
		return -EBUSY;
	}

	CL90135_emifA_fpga.aemif.addr = ioremap_nocache(DA8XX_EMIF25_CONTROL_BASE,SZ_32K);
	if (CL90135_emifA_fpga.aemif.addr == NULL) {
		printk(KERN_ERR	"%s: could not remap AEMIF control region\n",       __func__);
		return -ENOMEM;
	}
	...
	/* Set maximum wait cycles */
	writel(0xff, CL90135_emifA_fpga.aemif.addr + ASYNC_EMIF_AWCCR);

	/* Set the async memory config register for fpga: 16 bit memory */
	writel((0<<31)|(0<<30)|(0<<26)|(18<<20)|(0<<17)|(0<<13)|(18<<7)|(0<<4)|(2<<2)|1, CL90135_emifA_fpga.aemif.addr + ASYNC_EMIF_A1CR);

	/* Set the async memory config register for the latch */
	writel(0x00300388, CL90135_emifA_fpga.aemif.addr + ASYNC_EMIF_A2CR);
...
static __init void da8xx_evm_init(void)
{
    ...
    #define defGPIO_PerBankEnable       0x01E26008
    #define defGPIO_Bank45_Direction    0x01E26060
    #define defGPIO_Bank45_RisingEdge   0x01E26074
    #define defGPIO_Bank45_IntStat      0x01E26084
	
// better to keep GPIO_4/5/6/7 as GPIOs
// technical_reference_manual_MPU_spruh92c.pdf page 229
// PINMUX13_3_0 N2
//      0  Pin is 3-stated.
//      1h Selects Function AXR1[4]
//      2h Selects Function EQEP1B
//      3h-7h Reserved
//      8h Selects Function GP4[4] <-- THIS IS THE ONE WE GOT TO USE
//      9h-Fh Reserved
//
// and like this go the others settings
// PINMUX13_7_4 N1
//      8h Selects Function GP4[5]
// PINMUX13_11_8 M4
//      8h Selects Function GP4[6]
// PINMUX13_15_12 M3
//      8h Selects Function GP4[7]
//
// so finally the setting for the lower 15 bits of PINMUX13 must be: 0x8888

 
//    #define defIdeal_PINMUX13 0x11111111
// Michele: changed defIdeal_PINMUX13 to 0x11118888 to keep GPIO_4/5/6/7 as GPIOs, they pilot the board LEDs!
    #define defIdeal_PINMUX13 0x11118888
    #define defIdeal_PINMUX14 0x11111111
    #define defIdeal_PINMUX15 0x11111111
    #define defIdeal_PINMUX16 0x11111111
    #define defIdeal_PINMUX17 0x11111111
    #define defIdeal_PINMUX18 0x11111111
    #define defIdeal_PINMUX19 0x00000001
    
    #define AEMIF_A1CR              (0x68000010 )
    // Setup CS2 - 16-bit normal async 
    //    AEMIF_A1CR = 0x00C00609; // 16 bit memory!!!! 12 cycles in read and write
    // attenzione molto importante: minimum turnaround time deve essere maggiore o eguale ad 1 (qui impostato 2)
    // in modo da dare tempo ad fpga di disabilitarsi
    #define defSetup_AEMIF_A1CR  (0<<31)|(0<<30)|(0<<26)|(18<<20)|(0<<17)|(0<<13)|(18<<7)|(0<<4)|(2<<2)|1
...
// inizializzazione pinmux	
    davinci_cfg_writel(defIdeal_PINMUX13, DA8XX_PINMUX(13));
    davinci_cfg_writel(defIdeal_PINMUX14, DA8XX_PINMUX(14));
    davinci_cfg_writel(defIdeal_PINMUX15, DA8XX_PINMUX(15));
    davinci_cfg_writel(defIdeal_PINMUX16, DA8XX_PINMUX(16));
    davinci_cfg_writel(defIdeal_PINMUX17, DA8XX_PINMUX(17));
    davinci_cfg_writel(defIdeal_PINMUX18, DA8XX_PINMUX(18));
    davinci_cfg_writel(defIdeal_PINMUX19, DA8XX_PINMUX(19));
// setup emifa    
    CL90135_emifA_fpga_init();
//    davinci_cfg_writel(defSetup_AEMIF_A1CR, AEMIF_A1CR);

// MICHELE: IMPOSTO GPIO 5_6 IN INGRESSO, 
// è il segnale di interrupt request da sd acrd
//#define SD_CARD_INTERRUPT_PIN	GPIO(5*16+6)	// GPIO5[6]
	...
	addr = ioremap_nocache(defGPIO_Bank45_IntStat,SZ_32K);
	if (addr == NULL) {
		printk(KERN_ERR	"%s: could not remap intstat control region\n",       __func__);
		return;
	}
	i=readl(addr);
	writel(i|(1<<22), addr);
	
	addr = ioremap_nocache(defGPIO_PerBankEnable,SZ_32K);
	if (addr == NULL) {
		printk(KERN_ERR	"%s: could not remap intstat control region\n",       __func__);
		return;
	}
...
#if 1    
    // pin 5_6 rising edge interrupt
    //i=readl(defGPIO_Bank45_RisingEdge);
	//writel(i|(1<<22), defGPIO_Bank45_RisingEdge);
	// reset pending interrupts
#endif	

  
	
// MICHELE: IMPOSTO GPIO 2_13 IN USCITA, AL VALORE 1, IN MODO DA TOGLIERE RESET AD FPGA
// usare questa define per far fare al segnale reset fpga un toggle 1->0->1
//#define def_ON_BD_RESET_FPGA_DEBUG
#if 1
	if (gpio_request(ON_BD_RESET_FPGA, "ON_BD_RESET_FPGA")) {
		printk(KERN_ERR "%s: could not request GPIOs for RESET FPGA port \n", __func__);
		return;
	}
	
    gpio_direction_output(ON_BD_RESET_FPGA, 1);
    gpio_set_value(ON_BD_RESET_FPGA, 1);
#ifdef def_ON_BD_RESET_FPGA_DEBUG
	printk(KERN_WARNING "DA8xx: testing set to 1.\n");
    if (gpio_get_value(ON_BD_RESET_FPGA)!=1)
	    printk(KERN_WARNING "DA8xx: gpio 2_13, unable to set to 1.\n");
	    
	printk(KERN_WARNING "DA8xx: testing reset to 0.\n");
    gpio_set_value(ON_BD_RESET_FPGA, 0);
    if (gpio_get_value(ON_BD_RESET_FPGA)!=0)
	    printk(KERN_WARNING "DA8xx: gpio 2_13, unable to reset to 0.\n");

	printk(KERN_WARNING "DA8xx: testing set to 1 again.\n");
    gpio_set_value(ON_BD_RESET_FPGA, 1);
    if (gpio_get_value(ON_BD_RESET_FPGA)!=1)
	    printk(KERN_WARNING "DA8xx: gpio 2_13, unable to set again to 1.\n");
#endif	
    
	printk(KERN_WARNING "CL90135: gpio2_13=FPGA_RESET* set as output, value=1 (NOT ACTIVE).\n");
    gpio_free(ON_BD_RESET_FPGA);
#else	
	printk(KERN_WARNING "DA8xx: gpio 2_13 not set as output.\n");
#endif	
    // inizializzazione leds...
    for (i=0;i<sizeof(init_CL90135_leds)/sizeof(init_CL90135_leds[0]);i++){
	    if (gpio_request(init_CL90135_leds[i].uiGPIO, init_CL90135_leds[i].name)) {
		    printk(KERN_ERR "%s: could not request GPIOs for %s \n", __func__,init_CL90135_leds[i].name);
		    return;
	    }
        gpio_direction_output(init_CL90135_leds[i].uiGPIO, 1);
        gpio_set_value(init_CL90135_leds[i].uiGPIO, init_CL90135_leds[i].uiValue);
        gpio_free(init_CL90135_leds[i].uiGPIO);
    }
...
static const short da8xx_emifA_data16_pins[] = {
	...
	DA8XX_EMA_SDCKE, DA8XX_NEMA_CS_5, DA8XX_NEMA_WE,            //DA8XX_NEMA_CS_4 used for fpga interrupt
...
// Michele: sd card through fpga interface, not through dsp...
#if 0
...
{
	da8xx_psc0_pins[DA8XX_LPSC0_EMIF25] = da8xx_emifA_data16_pins;
// Michele: MMC viene usata tramite interfaccia FPGA...
...
// Michele: non uso mc asp in scheda cl90135
...
{
	printk(KERN_WARNING "CL90135: doing pinmux override.\n");
